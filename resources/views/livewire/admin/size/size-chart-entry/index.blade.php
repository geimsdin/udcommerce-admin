<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <flux:heading size="xl">{{ __('ecommerce::size-chart-entries.title') }}</flux:heading>
            <flux:subheading>{{ __('ecommerce::size-chart-entries.subtitle') }}</flux:subheading>
        </div>
        <flux:button variant="primary" icon="plus" :href="route(config('ud-ecommerce.admin_route_prefix', 'admin').'.size-chart-entries.create')" wire:navigate>
            {{ __('ecommerce::size-chart-entries.add_size_chart_entry') }}
        </flux:button>
    </div>
    ...
    <div x-data="{ show: false }"
         ...
         x-cloak>
        <flux:callout variant="success" icon="check-circle">
            {{ __('ecommerce::size-chart-entries.size_chart_entry_deleted') }}
        </flux:callout>
    </div>
    ...
    <flux:card class="space-y-4">
        ...
        <div class="max-w-sm">
            <flux:input
                ...
                placeholder="{{ __('ecommerce::size-chart-entries.search_placeholder') }}"
        ...
        <flux:table>
            <flux:table.columns>
                <flux:table.column>{{ __('ecommerce::size-chart-entries.table.id') }}</flux:table.column>
                <flux:table.column>{{ __('ecommerce::size-chart-entries.table.converted_value') }}</flux:table.column>
                <flux:table.column>{{ __('ecommerce::size-chart-entries.table.size_chart') }}</flux:table.column>
                <flux:table.column>{{ __('ecommerce::size-chart-entries.table.variant') }}</flux:table.column>
                <flux:table.column>{{ __('ecommerce::size-chart-entries.table.target_unit') }}</flux:table.column>
                <flux:table.column class="w-32">{{ __('ecommerce::size-chart-entries.table.actions') }}</flux:table.column>
            </flux:table.columns>
                        ...
                        <flux:table.column>
                            <div class="flex items-center gap-1">
                                ...
                                <flux:button
                                    ...
                                    wire:confirm="{{ __('ecommerce::size-chart-entries.delete_confirm') }}"
                    ...
                    <flux:table.row>
                        <flux:table.cell colspan="4" class="text-center py-8">
                            <div class="flex flex-col items-center gap-2">
                                ...
                                <flux:text class="text-zinc-500">{{ __('ecommerce::size-chart-entries.no_found') }}</flux:text>
                            </div>
                        </flux:table.cell>
                    </flux:table.row>
                @endforelse
