<div>
    ...
    <div class="flex items-center justify-between mb-4">
        <div>
            <flux:heading size="xl">
                {{ $isEditing ? __('ecommerce::brands.edit_brand') : __('ecommerce::brands.create_brand') }}
            </flux:heading>
            <flux:subheading>
                {{ $isEditing ? __('ecommerce::brands.messages.edit_subtitle') : __('ecommerce::brands.messages.create_subtitle') }}
            </flux:subheading>
        </div>
    </div>
    ...
    <flux:card>
        <form wire:submit="save" class="space-y-6">
            {{-- Basic Information --}}
            <flux:input wire:model="name" :label="__('ecommerce::brands.form.name')"
                placeholder="{{ __('ecommerce::brands.form.name_placeholder') }}" :badge="__('common.required')" />

            <flux:textarea wire:model="description" :label="__('ecommerce::brands.form.description')"
                placeholder="{{ __('ecommerce::brands.form.description_placeholder') }}" rows="3" />
            ...
            <div>
                ...
                @if ($image || $existingMediaUrl || $existingImage)
                    <div class="mt-2">
                            ...
                            <div
                                ...
                                @elseif ($existingImage)
                                    <img src="{{ asset('storage/' . $existingImage) }}"
                                        class="max-h-full max-w-full object-contain" alt="Brand logo" />
                                @endif
                            </div>
                        ...
                        @if (!empty($existingConversions) && !$image)
                            <div class="mt-3">
                                <span
                                    class="text-xs font-medium text-zinc-500 dark:text-zinc-400 mb-2 block">{{ __('ecommerce::brands.form.thumbnail_preview') }}:</span>
                                    ...
                                    @foreach ($existingConversions as $conversionName => $conversionUrl)
                                        <div class="text-center">
                                            <img src="{{ $conversionUrl }}"
                                                ...
                                                alt="{{ $conversionName }}" />
                                            <span class="text-xs text-zinc-400 mt-1 block font-mono">{{ $conversionName }}</span>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <flux:input wire:model="company_name" :label="__('ecommerce::brands.form.company_name')"
                    placeholder="{{ __('ecommerce::brands.form.company_name_placeholder') }}" />

                <flux:input wire:model="email" type="email" :label="__('ecommerce::brands.form.email')"
                    placeholder="{{ __('ecommerce::brands.form.email_placeholder') }}" />

                <flux:input wire:model="tel" type="tel" :label="__('ecommerce::brands.form.tel')"
                    placeholder="{{ __('ecommerce::brands.form.tel_placeholder') }}" />

                <flux:input wire:model="address" :label="__('ecommerce::brands.form.address')"
                    placeholder="{{ __('ecommerce::brands.form.address_placeholder') }}" />

                <flux:input wire:model="city" :label="__('ecommerce::brands.form.city')"
                    placeholder="{{ __('ecommerce::brands.form.city_placeholder') }}" />

                <flux:input wire:model="state" :label="__('ecommerce::brands.form.state')"
                    placeholder="{{ __('ecommerce::brands.form.state_placeholder') }}" />

                <flux:input wire:model="country" :label="__('ecommerce::brands.form.country')"
            ...
            <div class="flex items-center gap-4 justify-end">
                <flux:button variant="ghost"
                    :href="route(config('ud-ecommerce.admin_route_prefix', 'admin').'.brands.index')" wire:navigate>
                    {{ __('common.cancel') }}
                </flux:button>
