        ...
        <flux:table :paginate="$carriers">
            ...
            <flux:table.rows>
                @forelse ($carriers as $carrier)
                    <flux:table.row>
                        <flux:table.cell>{{ $carrier->name }}</flux:table.cell>
                        <flux:table.cell>{{ $carrier->price }}</flux:table.cell>
                        <flux:table.cell>
                            <flux:badge color="{{ $carrier->active ? 'green' : 'red' }}" icon="{{ $carrier->active ? 'check-circle' : 'x-circle' }}">
                                {{ $carrier->active ? __('common.yes') : __('common.no') }}
                            </flux:badge>
                        </flux:table.cell>
                        <flux:table.cell>
                            <div class="flex items-center gap-1">
                                <flux:button icon="pencil" :href="route(config('ud-ecommerce.admin_route_prefix', 'admin').'.carriers.edit', $carrier)" wire:navigate>
                                    {{ __('common.edit') }}
                                </flux:button>
                                <flux:button variant="danger" icon="trash" wire:click="requestDelete({{ $carrier->id }})">
                                    {{ __('common.delete') }}
                                </flux:button>
                            </div>
                        </flux:table.cell>
